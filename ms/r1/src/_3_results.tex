...
\begin{table}[ht]
    \caption{Summary of YOLO11 results for all configurations between the two model size variants. The results are represented as mean $\pm$ standard deviation. The greater mean value for each pair of model size variants is highlighted in bold.}
    \centering
    {\footnotesize
...
The benefits of transferring weights from a related task vary between the two evaluated datasets. On the more similar dataset, \textit{ext\_VT}, models initialized with transferred weights consistently outperformed those with pre-trained weights, with statistically significant differences except for the small YOLOv11 variant. Although the performance gap decreased as the training samples from the new source increased, the advantage of transferred weights remained when training with the full dataset of 200 images for most cases. Each model architecture exhibited the advantage by 2.62\% (RT-DETR-x), 1.76 \% (YOLOv12-x), and 0.98 \% (YOLOv11-x) in $\text{mAP@{0.5:0.95}}$ comparing the transferred weights with pre-trained weights. It is also found that the larger model variants tends to benefit more from transferred weights than the smaller ones. For instance, the advantage with the transferred weights was 2.62\% for the large RT-DETR model, while the small variant only gained 1.02\%. Similar trends were observed for YOLOv12 and YOLOv11, although the differences were smaller (1.03\% and 0.68\%, respectively).

In contrast, for the more distinct \textit{ext\_IMU} dataset, where the barn setting differs considerably, the advantage of transferred weights was observed only when training data was limited (e.g., 16 images). When the sample size increased to 200, the performance difference between pre-trained and transferred weights disappeared across all model architectures.
