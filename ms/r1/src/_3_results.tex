...
\subsection{Why DETR is More Data-Hungry than YOLO}
Figures~\ref{fig:multi_panel_small} and~\ref{fig:multi_panel_large} show that DETR consistently underperforms YOLOv11 and YOLOv12 at small training sizes (e.g., 32 or 64 images), but begins to close the gap when the dataset size reaches 128 or more images. However, even with further increases in training data, DETR's performance does not improve at the same rate as YOLO models. This plateauing effect suggests that DETR's data hunger is not only about needing more samples to reach parity, but also about its inefficiency in utilizing increasing data volumes effectively compared to CNN-based models.

This behavior is attributed to DETR's transformer-based architecture, which lacks the inductive spatial biases (e.g., local receptive fields, translation invariance) of CNNs. YOLO models use convolutional backbones  that make them efficient learners even from limited data. In contrast, DETR uses global self-attention and a set-based Hungarian matching loss to learn object detection, which requires significantly more training data and steps to converge. Moreover, the absence of intermediate object proposals and region-based priors may limit its capacity to benefit proportionally from more data in tasks that involve occlusions, background clutter, or class imbalance. As shown in prior research~\cite{carion2020end}, DETR achieves competitive performance only under longer training schedules and larger datasets, making it inherently more data-hungry and slower to scale.

\subsection{Why YOLOv12 Does Not Follow the Parameter vs. Step-Time Trend}

As illustrated in Figure~\ref{fig:time_vs_param}, most models exhibit a linear relationship between the number of parameters and training step time. However, YOLOv12 breaks this pattern. Despite having more parameters than YOLOv11, it achieves comparable or even lower median training step times across both NVIDIA A100 and L40s GPUs.

This deviation suggests that YOLOv12 incorporates architectural improvements that optimize GPU utilization. These may include fused convolutional operations, better activation functions, and efficient memory access strategies that reduce kernel launch overheads. Unlike raw parameter count, these implementation-level factors significantly affect training speed. As noted by Justus et al.~\cite{justus2018predicting}, compute time is often dictated more by tensor memory access and GPU-parallel-friendly design than by sheer model size. YOLOv12 thus offers a better trade-off between model complexity and computational efficiency, making it more suitable for scalable training in real-world settings.

\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{fig2.png}
\caption{}
\label{fig:multi_panel_small}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{fig3.png}
\caption{Performance scaling across four complex datasets. DETR performance improves significantly with more training data, reducing the gap with YOLO models.}
\label{fig:multi_panel_large}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{fig5.png}
\caption{Median training step time vs. number of model parameters across GPU types. YOLOv12 deviates from the linear trend, indicating improved training efficiency despite higher parameter count.}
\label{fig:time_vs_param}
\end{figure}
